...
# attempts still cost tokens: their estimate rides on the final msg's _usage so the
...
def chat_stream(msgs, route=None, rules=None):
    ...
    _events=_stream_events(msgs, route)
    try:
...
def _chat_stream_locked(msgs, rules, retries, route=None):
    ...
    for attempt in range(retries+1):
        try:
            ...
            try:
                msg=chat_stream(msgs, route, rules)
            finally:
                CHAT_LOCK.release()
            ...
            if (isinstance(e, urllib.error.HTTPError) and route and route["role"]=="escalate"
                ...
                return _chat_stream_locked(msgs, rules, retries, route=route)
            sys.stdout.write("\r\033[K")
            if attempt < retries:
...
def chat_stream_safe(msgs, retries=1, route=None):
    ...
    while True:
        try:
            msg=_chat_stream_locked(wire, STREAM_RULES, retries, route)
            ...
            continue
        # Only touch the count when an abort actually cost something: a clean turn whose
        # transport reported nothing must stay None (turn() logs unknown as null, not 0).
        if aborted_tokens:
            msg["_usage"]=(msg.get("_usage") or 0)+aborted_tokens
        return msg
